...
export function useMap() {
  ...
  const zoomY = useMemo(() => (viewportSize[1] - halfsqrt3 - 2 * margin) / sqrt3, [viewportSize]);
  
  const applyZoom = useCallback((zoom: number, { y }: SizeType) => {
  ...
  }, [center.x, center.y, dragBox.maxX, dragBox.maxY, dragBox.minX, dragBox.minY, zoomX, zoomY]);
  ...
  const dragTo = useCallback(([offsetX, offsetY]: [number, number]) => {
  ...
  const changeZoom = useCallback((zoom: number) => {
    applyZoom(zoom, size); // Changes viewport
    setZoomX(zoom); // Changes zoomX
  }, [applyZoom, size]);
